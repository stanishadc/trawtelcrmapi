...
namespace TrawtelCRMAPI.Helpers
{
    public class PaginationHelper
    {
        ...
        public static PagedFlightResponse<List<T>> CreateFlightPagedReponse<T>(List<T> pagedData, PaginationFilter validFilter, int totalRecords, IUriService uriService, string route, FlightRequestDTO flightRequestDTO)
        {
            var response = new PagedFlightResponse<List<T>>(pagedData, validFilter.PageNumber, validFilter.PageSize);
            var totalPages = ((double)totalRecords / (double)validFilter.PageSize);
            int roundedTotalPages = Convert.ToInt32(Math.Ceiling(totalPages));
            response.NextPage =
                validFilter.PageNumber >= 1 && validFilter.PageNumber < roundedTotalPages
            ...
            response.TotalPages = roundedTotalPages;
            response.TotalRecords = totalRecords;
            response.FlightRequestDTO = flightRequestDTO;
            return response;
        }
    }
}
